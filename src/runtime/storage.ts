...
function validatePath(p: string): void {
  if (!p || INVALID_PATH_RE.test(p)) {
    throw new Error(
      `Invalid path: "${p}". Must not contain "..", "//", or start with "/".`
    );
  }
}
...
export async function listFiles(
  path?: string,
  recursive = false
): Promise<{ path: string; files: FileInfo[]; folders: string[] }> {
  if (path) validatePath(path);
  const pfx = path
    ? fullKey(path).replace(/\/?$/, "/")
    : prefix()
      ? prefix() + "/"
      : "";

  const params: any = { Bucket: bucket(), Prefix: pfx, MaxKeys: 1000 };
  ...
  const files: FileInfo[] = (result.Contents ?? [])
    .filter((obj) => obj.Key !== pfx)
    .map((obj) => ({
      name: stripPrefix(obj.Key!).replace(/^.*\//, ""),
      size: obj.Size ?? 0,
